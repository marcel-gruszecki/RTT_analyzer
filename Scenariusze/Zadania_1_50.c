/**
 * @file scenario_1_50.c
 * @brief Scenariusz Testowy 1 - Idealna Symetria (n = 50 zadań)
 *
 * Projekt: RTT-Task Analyser
 * Autor: Marcel Gruszecki (UAM)
 * Opis: Implementuje scenariusz testowy generujący 50 unikalnych zadań w modelu Spdp-Any.
 * Faza sekwencyjna składa się z 30 zadań mono trwających po 10 ms, uruchomionych
 * idealnie symetrycznie (po 15 zadań na Rdzeń 0 oraz Rdzeń 1, co daje łącznie 150 ms).
 * Faza równoległa generuje 20 zadań dualnych o łącznym czasie barierowym 400 ms
...
/* ── RDZEŃ 0: NIESKOŃCZONA PĘTLA GŁÓWNA (Symetria, n=50) ────────────────── */
static void task_runner_core0(void *arg) {
    ...
    while (1) {
        ...
        /* Faza zadań mono na rdzeniu 0 - łącznie 150 ms */
        execute_mono_task("m0_t01", 10000); execute_mono_task("m0_t02", 10000);
        execute_mono_task("m0_t03", 10000); execute_mono_task("m0_t04", 10000);
        execute_mono_task("m0_t05", 10000); execute_mono_task("m0_t06", 10000);
        execute_mono_task("m0_t07", 10000); execute_mono_task("m0_t08", 10000);
        execute_mono_task("m0_t09", 10000); execute_mono_task("m0_t10", 10000);
        execute_mono_task("m0_t11", 10000); execute_mono_task("m0_t12", 10000);
        execute_mono_task("m0_t13", 10000); execute_mono_task("m0_t14", 10000);
        execute_mono_task("m0_t15", 10000);

        /* Faza zadań dualnych - 20 zadań z barierą synchronizacyjną, łącznie 400 ms */
        ...
        execute_dual_task("d_t13_0_d", "d_t13_1_d", 40000); execute_dual_task("d_t14_0_d", "d_t14_1_d", 40000);
        execute_dual_task("d_t15_0_d", "d_t15_1_d", 40000); execute_dual_task("d_t16_0_d", "d_t16_1_d", 40000);
        execute_dual_task("d_t17_0_d", "d_t17_1_d", 40000); execute_dual_task("d_t18_0_d", "d_t18_1_d", 40000);
        execute_dual_task("d_t19_0_d", "d_t19_1_d", 40000); execute_dual_task("d_t20_0_d", "d_t20_1_d", 40000);
...
static void task_runner_core1(void *arg) {
    while (1) {
        ...
        execute_mono_task("m1_t01", 10000); execute_mono_task("m1_t02", 10000);
        execute_mono_task("m1_t03", 10000); execute_mono_task("m1_t04", 10000);
        execute_mono_task("m1_t05", 10000); execute_mono_task("m1_t06", 10000);
        execute_mono_task("m1_t07", 10000); execute_mono_task("m1_t08", 10000);
        execute_mono_task("m1_t09", 10000); execute_mono_task("m1_t10", 10000);
        execute_mono_task("m1_t11", 10000); execute_mono_task("m1_t12", 10000);
        execute_mono_task("m1_t13", 10000); execute_mono_task("m1_t14", 10000);
        execute_mono_task("m1_t15", 10000);
    }
}
