...
/* ── RDZEŃ 0: GŁÓWNY ZARZĄDCA PĘTLI (Scenariusz 5, n=5) ───────────────── */
static void task_runner_core0(void *arg) {
    vTaskDelay(pdMS_TO_TICKS(100));

    while (1) {
        // === SCENARIUSZ 5 (n = 5): WIĘKSZOŚĆ ZADAŃ DWURDZENIOWYCH I NIELICZNE POJEDYNCZE ===

        // 1. START FAZY MONO
        xTaskNotifyGive(h_core1);

        // Nieliczne, lekkie zadania Mono na Core 0
        execute_mono_task("fx2", 1000);
        execute_mono_task("fx3", 1000);

        // Oczekiwanie na wyrównanie barierowe (Core 0 czeka na zakończenie cięższego fx1 z Core 1)
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);

        // 2. FAZA DUAL PARZYSTE (1 para dla zadania mono fx1 - generująca główny profil dwurdzeniowy)
        execute_dual_task("fx1_0_d", "fx1_1_d", 24000); // Ciężkie zadanie dualne (barierowo 12 ms)

        // 3. FAZA DUAL TŁO (1 czyste zadanie dualne jako uzupełnienie tła barierowego)
        execute_dual_task("fx4_0_d", "fx4_1_d", 16000); // Zadanie dualne tła (barierowo 8 ms)
