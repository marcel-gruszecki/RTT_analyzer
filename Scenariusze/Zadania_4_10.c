/**
 * @file scenario_4_10.c
 * @brief Scenariusz Testowy 4 - Wysoka Zmienność i Przeplatanie Obciążeń (n = 10 zadań)
 *
 ...
 * Cechą charakterystyczną jest wysoka zmienność (wariancja) czasów wykonania procesów.
 * W fazie mono oba rdzenie naprzemiennie wykonują zadania krótkie, średnie oraz długie
 * (fx1-fx3 na Rdzeniu 1, fx4-fx6 na Rdzeniu 0). Faza dualna kontynuuje schemat przeplatania
 * skrajnych wartości obciążeń (barierowo kolejno: 7 ms, 1 ms, 5 ms) uzupełniona o jedno
 * krótkie zadanie tła (2 ms).
 */
...
static void task_runner_core1(void *arg) {
    while (1) {
        ...
        /* Zadania mono na rdzeniu 1 - celowe przeplatanie ekstremalnych wartości */
        execute_mono_task("fx1", 1000);
        execute_mono_task("fx2", 16000);
        execute_mono_task("fx3", 2000);

        xTaskNotifyGive(h_core0);
    }
}

/* ── RDZEŃ 0: GŁÓWNY ZARZĄDCA PĘTLI (Scenariusz 4, n=10) ───────────────── */
static void task_runner_core0(void *arg) {
    ...
    while (1) {
        ...
        /* Zadania mono na rdzeniu 0 - przeplatanie długości obciążeń */
        execute_mono_task("fx4", 12000);
        execute_mono_task("fx5", 1000);
        execute_mono_task("fx6", 8000);

        /* Oczekiwanie na wyrównanie barierowe z rdzeniem 1 */
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);

        /* Faza zadań dualnych - wysoka zmienność czasów oraz przeplatanie długości */
        execute_dual_task("fx1_0_d", "fx1_1_d", 14000);
