...
static void task_runner_core1(void *arg) {
    while (1) {
        ...
        // Rdzeń 1: Wykonuje tylko jedno ultra krótkie zadanie Mono i czeka bezczynnie
        execute_mono_task("fx3", 1000); // 1 ms

        xTaskNotifyGive(h_core0);
    }
}

/* ── RDZEŃ 0: GŁÓWNY ZARZĄDCA PĘTLI (Scenariusz 6, n=5) ───────────────── */
static void task_runner_core0(void *arg) {
    vTaskDelay(pdMS_TO_TICKS(100));

    while (1) {
        // === SCENARIUSZ 6 (n = 5): JEDEN RDZEŃ SEKWENCJA CORAZ DŁUŻSZYCH ZADAŃ ===

        // 1. START FAZY MONO
        xTaskNotifyGive(h_core1);

        // Rdzeń 0: Sekwencja rosnących monolitycznych zadań
        execute_mono_task("fx1", 4000);  // Średnie zadanie (4 ms)
        execute_mono_task("fx2", 12000); // Bardzo długie zadanie (12 ms) - Rdzeń 1 w tym czasie marnuje zasoby

        // Oczekiwanie na wyrównanie barierowe (Rdzeń 1 dawno skończył fx3 i czekał bezczynnie)
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);

        // 2. FAZA DUAL PARZYSTE (1 para sparowana z zadaniem mono fx1)
        execute_dual_task("fx1_0_d", "fx1_1_d", 4000);  // barierowo 2 ms

        // 3. FAZA DUAL TŁO (1 czyste zadanie dualne bez odpowiednika mono)
        execute_dual_task("t1_0_d", "t1_1_d", 6000);    // barierowo 3 ms
