...
static void task_runner_core1(void *arg) {
    while (1) {
        ...
        execute_mono_task("fx3", 4000);  // 4 ms
        execute_mono_task("fx4", 4000);  // 4 ms (łącznie 10 ms, potem długa bezczynność)

        xTaskNotifyGive(h_core0);
    }
}

/* ── RDZEŃ 0: GŁÓWNY ZARZĄDCA PĘTLI (Scenariusz 6, n=10) ───────────────── */
static void task_runner_core0(void *arg) {
    vTaskDelay(pdMS_TO_TICKS(100));

    while (1) {
        // === SCENARIUSZ 6 (n = 10): JEDEN RDZEŃ SEKWENCJA CORAZ DŁUŻSZYCH ZADAŃ ===

        // 1. START FAZY MONO
        xTaskNotifyGive(h_core1);

        // Rdzeń 0: Sekwencja rosnących monolitycznych zadań (łącznie aż 28 ms)
        execute_mono_task("fx1", 2000);  // Krótkie zadanie (2 ms)
        execute_mono_task("fx5", 8000);  // Średnie zadanie (8 ms)
        execute_mono_task("fx6", 18000); // Bardzo długie zadanie (18 ms) - Rdzeń 1 czeka bezczynnie

        // Oczekiwanie na wyrównanie barierowe (Rdzeń 1 dawno skończył i czeka)
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);

        // 2. FAZA DUAL PARZYSTE (3 pary dla istniejących zadań mono fx1, fx2, fx3)
        execute_dual_task("fx1_0_d", "fx1_1_d", 6000);   // barierowo 3 ms
        execute_dual_task("fx2_0_d", "fx2_1_d", 4000);   // barierowo 2 ms
        execute_dual_task("fx3_0_d", "fx3_1_d", 12000);  // barierowo 6 ms

        // 3. FAZA DUAL TŁO (1 czyste zadanie dualne bez odpowiednika mono)
        execute_dual_task("t1_0_d", "t1_1_d", 8000);     // barierowo 4 ms
