...
static void task_runner_core1(void *arg) {
    while (1) {
        ...
        execute_mono_task("fx3", 10000);
        execute_mono_task("fx4", 10000);
        execute_mono_task("fx5", 10000);
        execute_mono_task("fx6", 10000);

        xTaskNotifyGive(h_core0);
    }
}

/* ── RDZEŃ 0: GŁÓWNY ZARZĄDCA PĘTLI (Scenariusz 5, n=30) ───────────────── */
static void task_runner_core0(void *arg) {
    vTaskDelay(pdMS_TO_TICKS(100));

    while (1) {
        // === SCENARIUSZ 5 (n = 30): WIĘKSZOŚĆ ZADAŃ DWURDZENIOWYCH I NIELICZNE POJEDYNCZE ===

        // 1. START FAZY MONO
        xTaskNotifyGive(h_core1);

        // Zadanie mono pod parę dualną na Core 0
        execute_mono_task("fx7", 500);

        // Zadania Mono na Core 0 (Lekkie zadania uzupełniające pulę unikalnych nazw)
        execute_mono_task("fx8", 500);   execute_mono_task("fx9", 500);
        execute_mono_task("fx10", 500);  execute_mono_task("fx11", 500);
        ...
        execute_mono_task("fx18", 500);

        // Oczekiwanie na wyrównanie barierowe (Core 0 szybko kończy i czeka na Core 1)
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);

        // 2. FAZA DUAL PARZYSTE (7 par dla istniejących zadań mono fx1-fx7)
        ...
        execute_dual_task("fx6_0_d", "fx6_1_d", 4000);
        execute_dual_task("fx7_0_d", "fx7_1_d", 4000);

        // 3. FAZA DUAL TŁO (5 czystych zadań dualnych jako uzupełnienie tła barierowego - Scenariusz 5)
