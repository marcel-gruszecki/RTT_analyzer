/**
 * @file scenario_3_10.c
 * @brief Scenariusz Testowy 3 - Odwrócona Asymetria / Sortowanie Rosnące (n = 10 zadań)
 *
 ...
 * Faza jednordzeniowa (Mono) jest rozdzielona na oba rdzenie i posortowana rosnąco według
 * czasu trwania (fx1, fx2 na Rdzeniu 0 oraz fx3, fx4 na Rdzeniu 1). Faza równoległa
 * (Dual) realizuje wykonanie zadań wariantu elastycznego oraz zadań tła, zachowując
 * ścisłe sortowanie rosnące (SJF - Shortest Job First) względem czasu barierowego.
 */
...
static void task_runner_core1(void *arg) {
    while (1) {
        ...
        /* Zadania mono na rdzeniu 1 sformatowane w porządku rosnącym */
        execute_mono_task("fx3", 6000);
        execute_mono_task("fx4", 8000);

        xTaskNotifyGive(h_core0);
    }
}

/* ── RDZEŃ 0: GŁÓWNY ZARZĄDCA PĘTLI (Scenariusz 3, n=10) ──────── */
static void task_runner_core0(void *arg) {
    ...
    while (1) {
        ...
        /* Zadania mono na rdzeniu 0 sformatowane w porządku rosnącym */
        execute_mono_task("fx1", 2000);
        execute_mono_task("fx2", 4000);
        ...
        execute_dual_task("fx2_0_d", "fx2_1_d", 8000);
        execute_dual_task("fx3_0_d", "fx3_1_d", 12000);
        execute_dual_task("fx4_0_d", "fx4_1_d", 16000);
        execute_dual_task("t2_0_d", "t2_1_d", 20000);
