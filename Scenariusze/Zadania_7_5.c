...
static void task_runner_core1(void *arg) {
    while (1) {
        ...
        // ZADANIE 5 (Jedno krótkie zadanie na Core 1, generujące lukę/przestój)
...
/* ── RDZEŃ 0: GŁÓWNA PĘTLA ZARZĄDZAJĄCA (Scenariusz 7, n=5) ───────────── */
static void task_runner_core0(void *arg) {
    ...
    while (1) {
        ...
        // ZADANIA 1 - 4 (Przeładowany Core 0 - 4 zadania po 15 ms = 60 ms)
        execute_mono_task("t1", 15000);
        execute_mono_task("t2", 15000);
        execute_mono_task("t3", 15000);
        execute_mono_task("t4", 15000);

        // Blokada pętli dopóki Core 1 nie potwierdzi zakończenia (choć skończył znacznie wcześniej)
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);

        // Bilans unikalnych zadań w logu z jednej iteracji:
        // t1, t2, t3, t4 na Core 0 + t5 na Core 1 = Dokładnie 5 unikalnych zadań mono.
        // Brak jakichkolwiek zadań dualnych.
