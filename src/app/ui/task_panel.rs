//! # Panel Monitorowania Zadań w Czasie Rzeczywistym
//!
//! Projekt: RTT-Task Analyser
//! Autor: Marcel Gruszecki (UAM)
//! Moduł: `app::ui::task_panel`
//! Opis: Odpowiada za renderowanie głównego widoku monitoringu RTOS.
//!       Prezentuje dynamiczną listę wykrytych procesów, oblicza bieżące obciążenie procentowe
//!       rdzeni procesora (CPU Utilisation) oraz rysuje ruchomy wykres Gantta czasu rzeczywistego.
...
/// Widoczne domyślnie okno czasowe na wykresie Gantta (5 sekund).
const WINDOW_US: u64 = 5_000_000;

/// Paleta odcieni niebieskich stosowana do kolorowania pasków zadań.
...
// Wylicza deterministyczny odcień koloru na podstawie nazwy zadania
...
/// Funkcja agregująca, budująca układ całego panelu zadań.
pub fn show_task_panel(
    ctx: &egui::Context,
    execs: &[TaskExecution],
    schedule_data: &[TaskScheduleData],
) {
    show_task_left_panel(ctx, execs, schedule_data);
    show_task_right_panel(ctx, execs, schedule_data);
    show_central_panel_task(ctx, execs);
}

/// Renderuje lewy panel boczny z listą unikalnych nazw aktywnych zadań.
pub fn show_task_left_panel(
    ctx: &egui::Context,
    execs: &[TaskExecution],
    _schedule_data: &[TaskScheduleData]
) {
                ...
                .show(ui, |ui| {
                    ...
                    for e in execs {
                        if seen.insert(e.name.as_str()) {
                            ui.horizontal(|ui| {
                                ui.label(egui::RichText::new("●").color(task_color(&e.name)));
                                ui.label(&e.name);
                            });
                        }
                    }
...
/// Renderuje prawy panel boczny z ogólną statystyką i obciążeniem rdzeni mikrokontrolera.
pub fn show_task_right_panel(
    ...
    egui::SidePanel::right("side_panel_right")
        ...
        .show(ctx, |ui| {
            ...
            let mut unique_tasks: std::collections::HashSet<&str> =
                schedule_data.iter().map(|s| s.name.as_str()).collect();

            if unique_tasks.is_empty() {
                unique_tasks = execs.iter().map(|e| e.name.as_str()).collect();
            }
            ...
            // Wyliczanie rzeczywistego obciążenia CPU (Total busy time / Wall time)
            if wall_us > 0 {
                let mut busy = [0u64; 2];
                for e in execs {
                ...
                for (i, &b) in busy.iter().enumerate() {
                    ...
                    ui.label(format!("CPU rdzeń {}: {:.1}%", i, pct));
                }
            } else {
                ui.label("CPU rdzeń 0: —");
                ui.label("CPU rdzeń 1: —");
            }
        });
}

/// Renderuje panel centralny z ruchomym wykresem Gantta i tabelą logów wejść/wyjść.
pub fn show_central_panel_task(ctx: &egui::Context, execs: &[TaskExecution]) {
    egui::CentralPanel::default().show(ctx, |ui| {
        ui.label(format!("Zakończone wykonania: {}", execs.len()));
        ui.separator();

        // Obliczanie granic ruchomego okna czasowego (Rolling window)
        ...
        Plot::new("gantt_task")
            ...
            .show(ui, |plot_ui| {
                ...
                // Rysowanie etykiet tekstowych wewnątrz bloków na wykresie Gantta
                for e in &visible {
        ...
        egui::Grid::new("exec_table_header").show(ui, |ui| {
            for h in &["Zadanie", "Start [us]", "Koniec [us]", "Rdzeń", "Czas [us]"] {
                ui.add_sized(
                    [col_w, 20.0],
        ...
        // Tabela logów (Scroll area z automatycznym przyklejaniem widoku do dołu)
        egui::ScrollArea::vertical()
                ...
                egui::Grid::new("exec_table_rows")
                    ...
                    .show(ui, |ui| {
                        for e in execs {
                            ui.add_sized(
                                [col_w, 18.0],
                                egui::Label::new(egui::RichText::new(&e.name).color(task_color(&e.name))),
                            );
                            ...
                            ui.add_sized([col_w, 18.0], egui::Label::new(e.duration_us().to_string()));
                            ui.end_row();
                        }
                    });
            });
